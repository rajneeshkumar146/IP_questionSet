...
/**
 * Series starting from 1, given an array of natural numbers delete these numbers
 * and print the kth smallest from remaining.
 */
	...
	private static void ksmallest(int[] arr, int k) {
		...
		// all elements < k can be ignored, we shift the start at k
		if (k > arr[arr.length - 1]) {
			System.out.println(k + arr.length);
			return;
		}
		// count gap between first element to be deleted and 1
		int count = arr[0] - 1;
		if (k <= count) {
	...
	/**
	 * prints k smallest elements starting [i+1 to j-1]
	 */
